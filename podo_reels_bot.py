...
def add_cors_headers(response):
    """Разрешаем запросы с любого origin (для генератора на Netlify)"""
...
def generate_ideas():
    ...
    try:
        ...
        # Отправляем запрос к Claude напрямую через HTTP (без anthropic SDK)
        ...
        if not api_response.ok:
            logger.error(f"Anthropic API error {api_response.status_code}: {api_response.text}")
            return jsonify({'error': f'API {api_response.status_code}: {api_response.text[:300]}'}), 500
        response_data = api_response.json()
        response_text = next(
            (block["text"] for block in response_data.get("content", []) if block.get("type") == "text"),
            None
        )
        if not response_text:
            logger.error(f"No text block in response: {response_data}")
            return jsonify({'error': 'No text content in API response'}), 500
...
def main() -> None:
    ...
    # (job_queue отключен из-за несовместимости версий - пароль меняется вручную)
